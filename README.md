...
| Sparse component storage _(v0.51.0, storage only)_ | `SetSparse(w, compID)` / `IsSparse(scope, compID)` / `EachSparse[T](scope, fn)` — per-component sparse-set; no archetype transition; pointer-stable; entity-delete cleanup; JSON round-trip; query integration pending Phase 15.20 |
