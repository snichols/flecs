...
| REST API _(v0.87.0)_ | `NewRESTHandler(w)` — read-only HTTP inspection + snapshot save/load (`GET /stats`, `/stats/world`, `/stats/pipeline`, `/components`, `/entities`, `/snapshot`, `/type_info/{path}`; `PUT /snapshot`). `GET /type_info/{path}` returns the reflection schema (size, align, fields) for any named component. |
