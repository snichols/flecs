...
| OnDelete / OnDeleteTarget observers _(v0.103.0)_ | `OnDelete(w, fn)` fires before entity deletion (before `OnRemove` hooks); `OnDeleteTarget(w, fn)` fires per (target, dependent, pairRelID) triple during cleanup-policy cascade. `WithOptions` variants support `WithQuery(terms...)` and `WithRelationship(relID)` filters. Handlers receive `*Reader` (entity mid-delete; defer mutations via `World.Write(fn)`). Component-remove cascade: deleting a component entity now actively removes it from all holders. See [docs/ObserversManual.md § OnDelete and OnDeleteTarget](docs/ObserversManual.md#on-delete-and-on-delete-target) |
...
| Streaming snapshot I/O _(v0.108.0)_ | `(*Snapshot).WriteTo(io.Writer)` (satisfies `io.WriterTo`) / `TakeSnapshotTo(w, out)` / `TakeSnapshotToContext(ctx, w, out)` — write directly to any writer without materializing a `[]byte`; `ReadSnapshotFrom(r)` / `RestoreSnapshotFrom(w, r)` / `RestoreSnapshotFromContext(ctx, w, r)` — read from any reader. Composes with `gzip.NewWriter`, `net.Conn`, `*os.File`. Format unchanged; `Bytes()` is now a thin wrapper. See [docs/Snapshots.md](docs/Snapshots.md#streaming-snapshots-v01080). |
...
| Context cancellation _(v0.106.0)_ | `w.ProgressContext(ctx, dt)`, `w.TakeSnapshotContext(ctx)`, `w.RestoreSnapshotContext(ctx, snap)`, `RunSystemContext(ctx, sys, dt)`, `q.EachContext(ctx, fn)`, `cq.EachContext(ctx, fn)`, `w.MarshalJSONContext(ctx)` — cooperative cancellation on all long-running operations; REST endpoints honour `r.Context()` and write 499 on disconnect; `context.Background()` callers pay zero overhead. See [docs/Cancellation.md](docs/Cancellation.md). |
| Range-over-func iteration _(v0.107.0)_ | `All1[A](s)` / `All2[A,B](s)` / `All3[A,B,C](s)` / `All4[A,B,C,D](s)` — return `iter.Seq2[ID, *A]` / `Pair` / `Triple` / `Quad` for idiomatic `for e, pos := range flecs.All1[Position](r)` loops with full break support (intra-table and inter-table). `QueryAll(q, s)` / `CachedQueryAll(cq, s)` for bare entity-ID `iter.Seq[ID]`. `QueryAllContext` / `CachedQueryAllContext` for context-aware `iter.Seq2[ID, error]`. All Inheritable (upPtr shared-pointer) and CanToggle (IsRowEnabled) paths preserved. See [docs/Queries.md](docs/Queries.md#range-over-func-iteration). |
| Traversal iter.Seq adapters _(v0.109.0)_ | `r.Entities()` / `r.Children(parent)` / `r.Prefabs(e)` / `r.Systems(phase)` — `iter.Seq` adapters for the `Reader`-level traversal primitives. `Union(s, relID)` / `Sparse[T](s)` / `ByID(s, id)` — package-level `iter.Seq2` adapters for payload-bearing traversals (entity + target/value); drive the underlying machinery directly for break support. `r.EntitiesContext(ctx)` / `ByIDContext(ctx, s, id)` — context-aware `iter.Seq2[ID, error]` with cancellation checked every 1024 entities/tables. |
| Testing helpers _(v0.110.0)_ | `flectest.NewWorld(tb)` / `flectest.NewWorldWith(tb, setup)` / `flectest.MustEntity(tb, w, name, comps...)` / `flectest.MustChild` — world and entity builders for tests. Assertions: `AssertAlive`, `AssertHasComponent[T]`, `AssertComponentValue[T]`, `AssertParent`, `AssertChildren`, `AssertHasPair`, `AssertName`, `AssertEntityCount`, `AssertQueryCount`. Snapshots: `AssertSnapshotGolden` (golden-file diff, `-update` rewrite) / `RequireRoundTrip` (JSON round-trip). Import `github.com/snichols/flecs/flectest` from `_test.go` only. See [docs/Testing.md](docs/Testing.md). |
| expvar metrics _(v0.111.0)_ | `flecs.PublishExpvar(w, prefix)` — publish entity/table/component/system/observer counts, frame count, reclaimed tables, last-progress timestamp, per-phase timing, and second/minute/hour window aggregates as lazy `expvar.Func` variables at `/debug/vars`; zero third-party dependencies. `(*ExpvarHandle).Unpublish()` nulls out vars. `ExpvarMap(w)` — live `*expvar.Map` for custom mounting. All reads goroutine-safe via `statsMu.RLock`. See [docs/Observability.md](docs/Observability.md). |
| Snapshot migration registry _(v0.112.0)_ | `w.SetSchemaVersion(v)` / `w.RegisterMigration(from, to, fn)` — schema-version tagging embedded in binary snapshots (format v3); migration chain transforms old component layouts before materialization; `*MigrationContext` operations: `RenameComponent`, `DropComponent`, `AddComponent`, `EachComponent` (byte rewrite via `ComponentRecord.SetRaw`); atomicity: decode + migrate + validate before any world mutation; `ErrMissingMigration` / `ErrSnapshotNewerThanWorld` sentinel errors; format v2 snapshots still accepted. See [docs/SnapshotMigration.md](docs/SnapshotMigration.md). |
