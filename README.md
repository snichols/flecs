...
| REST API _(v0.88.0)_ | `NewRESTHandler(w)` — HTTP inspection + entity mutation (`GET /stats`, `/stats/world`, `/stats/pipeline`, `/components`, `/entities`, `/snapshot`, `/type_info/{path}`; `PUT /snapshot`; `PUT /entity`, `DELETE /entity/{path...}`). `GET /type_info/{path}` returns the reflection schema (size, align, fields) for any named component. `PUT /entity` creates or claims an entity (JSON body: `id`, `name`, `parent`). `DELETE /entity/{path...}` deletes an entity by dot-separated path. |
