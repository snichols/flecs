...
func TestIsAWorldCountBaseline(t *testing.T) {
	...
	base := w.Count()
	if base != 35 {
		t.Fatalf("fresh World.Count(): want 35 (ChildOf + IsA + Name + PreUpdate + OnUpdate + PostUpdate + OnFixedUpdate + OnInstantiate + Inherit + Override + DontInherit + OnDelete + OnDeleteTarget + RemoveAction + DeleteAction + PanicAction + Exclusive + CanToggle + Symmetric + Transitive + Reflexive + Acyclic + Final + OneOf + Singleton + WriteOnce + Traversable + Relationship + Target + Trait + PairIsTag + With + OrderedChildren + Wildcard + Any), got %d", base)
	}
}
