...
func ExampleWorld_basic() {
	...
	e1 := w.NewEntity()
	e2 := w.NewEntity()
	flecs.Set(w.W(), e1, basicPos{X: 1, Y: 2})
	flecs.Set(w.W(), e1, basicVel{DX: 0.5})
	flecs.Set(w.W(), e2, basicPos{X: 4, Y: 0})
	flecs.Set(w.W(), e2, basicVel{DX: -1})

	// Read a single component.
	if p, ok := flecs.Get[basicPos](w.R(), e1); ok {
		fmt.Printf("e1 pos: %.0f %.0f\n", p.X, p.Y)
	}

	// Iterate all entities that have both Position and Velocity.
	flecs.Each2[basicPos, basicVel](w.R(), func(e flecs.ID, p *basicPos, v *basicVel) {
		p.X += v.DX // update in place
	})

	// Positions are updated.
	p1, _ := flecs.Get[basicPos](w.R(), e1)
	p2, _ := flecs.Get[basicPos](w.R(), e2)
