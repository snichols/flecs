...
- **Entity hooks beyond OnAdd/OnSet/OnRemove** — C has `OnDelete`, `OnTableEmpty`, `OnTableFill`. `OnTableCreate` observer event **shipped in v0.62.0**; `OnTableEmpty`/`OnTableFill` **shipped in v0.98.0**; `OnTableDelete` **shipped in v0.101.0** (requires table reclamation; see [TableReclamation.md](TableReclamation.md)).
...
- **Units addon** — ✅ **shipped in v0.85.0** via `RegisterUnit` / `(*World).UnitFor` / `(*Writer).SetUnit` / `Convert`. 15 built-in units (Length: Meter/KiloMeter/MilliMeter; Duration: Second/MilliSecond/Minute/Hour; Mass: Gram/KiloGram/MegaGram; Force: Newton; Energy: Joule; Frequency: Hertz; Angle: Radian/Degree); user-defined units with arbitrary base chains; multi-hop `Convert`; definitions survive JSON round-trip. Compound units (`m/s`, `kg·m²/s²`) ✅ **shipped in v0.97.0** (Phase 16.42). See [docs/Units.md](Units.md).
...
- **`OnTableDelete` event** — ✅ **shipped in v0.101.0** via `OnTableDelete(w, fn)` / `OnTableDeleteWithOptions(w, opts, fn)`. Fires synchronously before a table is reclaimed; handler receives `*Reader` (table is mid-destruction). Table reclamation runs at the start of each `Progress()` tick; default threshold is 60 empty ticks. See [ObserversManual.md § OnTableDelete](ObserversManual.md#on-table-delete) and [TableReclamation.md](TableReclamation.md).
- **`OnTableEmpty` / `OnTableFill` events** — ✅ **shipped in v0.98.0** via `OnTableEmpty(w, fn)` / `OnTableEmptyWithOptions(w, opts, fn)` / `OnTableFill(w, fn)` / `OnTableFillWithOptions(w, opts, fn)`. Fires when an archetype table transitions between empty (0 rows) and non-empty (≥1 rows). Supports multi-term filters and `WithYieldExisting()`. See [ObserversManual.md § OnTableEmpty / OnTableFill Events](ObserversManual.md#on-table-empty-and-on-table-fill).
