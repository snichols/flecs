...
- **Query variables** — ✅ **shipped in v0.80.0** via `WithVar(componentID, varName)` / `WithPairTgtVar(rel, varName)` / `(Term).SrcVar(name)` / `(Term).TgtVar(name)` / `(*QueryIter).Var(name) ID`. Single-variable v1: one named variable plus implicit `$this`; first-defined variable is the driver; driver domain enumerated at `Iter()` time; results pre-materialized per-row. Multi-variable join optimization deferred to Phase 16.25.x. See [Queries.md § Query variables](Queries.md#query-variables-single-variable-v1).
