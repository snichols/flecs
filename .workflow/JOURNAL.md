...
Fixed gofmt violation in world_test.go: removed double space before inline comments on lines 632-633 (changed `  //` to ` //`). golangci-lint now passes with no violations. All tests pass with -race flag.
