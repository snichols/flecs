...
Added two missing verification matrix tests: TestParallelSystems_OrderingPreserved (verifies within-stage FIFO submission order via OnAdd hook sequence [eA,eB] in stage 1, and across-stage merge order with eC in stage 2 firing last) and TestParallelSystems_NestedScopesBlocked (verifies stage.deferDepth>0 scope guards reject RangeSet from a worker goroutine inside a parallel system; two-system batch ensures worker-goroutine execution). go vet clean, golangci-lint clean, go test ./... -race -count=10 clean.
