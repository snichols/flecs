...
func TestCachedQuerySingleTerm(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		e := w.NewEntity()
		flecs.Set(w, e, Position{X: float32(i)})
	}
...
func TestCachedQueryIterWalksCachedTables(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		e := w.NewEntity()
		flecs.Set(w, e, Position{X: float32(i + 5)})
		flecs.Set(w, e, Velocity{DX: 2})
		flecs.Set(w, e, LocalMarker{})
	}
...
func TestCachedQueryInitialPopulation(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		e := w.NewEntity()
		flecs.Set(w, e, Position{X: float32(i)})
		flecs.Set(w, e, Velocity{DX: float32(i)})
	}
...
func TestCachedQueryMultiple(t *testing.T) {
	...
	// [P]-only entity.
	eP := w.NewEntity()
	flecs.Set(w, eP, Position{X: 1})
	// [V]-only entity.
	eV := w.NewEntity()
	flecs.Set(w, eV, Velocity{DX: 1})
	// [P,V] entity.
	ePV := w.NewEntity()
	flecs.Set(w, ePV, Position{X: 2})
	flecs.Set(w, ePV, Velocity{DX: 2})
	...
	flecs.Set(w, e, Velocity{DX: 3})
	flecs.Set(w, e, LocalM{})
...
func TestCachedQueryWithDefer(t *testing.T) {
	...
	w.DeferEnd() // flushes; e migrates to [P,V] → notifyTableCreated fires
...
func TestCachedQueryFieldT(t *testing.T) {
	...
	flecs.Set(w, e, Velocity{DX: 3, DY: 4})
