...
- **Sparse component storage** _(v0.51.0–v0.52.0, Phases 15.19–15.20)_ — `SetSparse(w, compID)` / `IsSparse(scope, compID)` / `w.Sparse()` (index 34) / `EachSparse[T](scope, fn)`. Data stored in per-component sparse-set (dense vector + sparse page index) rather than archetype columns. No archetype transition on add/remove. Pointer-stable boxed allocations (`reflect.New`). `HasID`/`OwnsID`/`GetRef`/`Get[T]` consult sparse-set. Entity-delete O(k) cleanup via `sparseHeld`. JSON round-trip (`sparse_policies` + `sparse_data` fields). Go flecs consolidates upstream `EcsSparse` + `EcsIdDontFragment` into a single trait. **Query integration (v0.52.0):** `NewQuery`/`NewCachedQuery` compile sparse terms; three-mode iterator (all-sparse, mixed, all-archetype); `Field[T]`/`FieldMaybe[T]` sparse branches; `Not`/`Optional` on sparse terms; `CachedQuery.Changed()` via version counter. Built-in entity count: 36; user entities now start at index 37.
