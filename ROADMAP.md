...
- **Sparse component storage (storage path)** _(v0.51.0, Phase 15.19)_ — `SetSparse(w, compID)` / `IsSparse(scope, compID)` / `w.Sparse()` (index 34) / `EachSparse[T](scope, fn)`. Data stored in per-component sparse-set (dense vector + sparse page index) rather than archetype columns. No archetype transition on add/remove. Pointer-stable boxed allocations (`reflect.New`). `HasID`/`OwnsID`/`GetRef`/`Get[T]` consult sparse-set. Entity-delete O(k) cleanup via `sparseHeld`. JSON round-trip (`sparse_policies` + `sparse_data` fields). Go flecs consolidates upstream `EcsSparse` + `EcsIdDontFragment` into a single trait for this phase. **Query integration deferred to Phase 15.20** — terms naming a Sparse component will not match in queries yet. Built-in entity count: 36; user entities now start at index 37.
