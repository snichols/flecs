...
- **Wildcard and Any query-term sentinels** _(v0.38.0)_ — `w.Wildcard()` (`*`, index 21) and `w.Any()` (`_`, index 22). `MakePair(R, w.Wildcard())` emits one iterator row per concrete `(R, X)` pair in the table; `MakePair(R, w.Any())` matches once per entity. `MatchedTarget(it, termIdx)` / `MatchedID(it, termIdx)` / `FieldByMatch[T](it, termIdx)` accessors. Works in both live queries and `CachedQuery`. Built-in entity count: 22; user entities now start at index 23.
