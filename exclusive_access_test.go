...
func TestGoidGetIsNonZero(t *testing.T) {
	...
	w.Write(func(fw *flecs.Writer) {
		flecs.Set(fw, e, xaPos{1, 1})
	})
	w.ExclusiveAccessEnd(false)
}

// TestExclusiveAccessZeroOverheadCommonPath is defined in exclusive_access_norace_test.go
// (//go:build !race) because the race detector's shadow memory instrumentation causes
// closure captures to escape to the heap, yielding spurious allocations.
