...
func TestOnSetRegistrationReplaces(t *testing.T) {
	w := flecs.New()
	calls := 0
	flecs.OnSet[Position](w, func(_ flecs.ID, _ *Position) { calls++ })
	flecs.OnSet[Position](w, func(_ flecs.ID, _ *Position) { calls += 10 })
	e := w.NewEntity()
...
func TestOnAddClearWithNil(t *testing.T) {
	w := flecs.New()
	calls := 0
	flecs.OnAdd[Position](w, func(_ flecs.ID, _ *Position) { calls++ })
...
func TestOnRemoveClearWithNil(t *testing.T) {
	w := flecs.New()
	calls := 0
	flecs.OnRemove[Position](w, func(_ flecs.ID, _ *Position) { calls++ })
...
func TestOnAddFiresOnceOnInitialSet(t *testing.T) {
	...
	flecs.OnAdd[Position](w, func(e flecs.ID, p *Position) { addCount++ })
	flecs.OnSet[Position](w, func(e flecs.ID, p *Position) { setCount++ })
	e := w.NewEntity()
	flecs.Set[Position](w, e, Position{1, 2})
	if addCount != 1 {
...
func TestOnSetReceivesCorrectValue(t *testing.T) {
	w := flecs.New()
	var got Position
	flecs.OnSet[Position](w, func(_ flecs.ID, p *Position) { got = *p })
	e := w.NewEntity()
	flecs.Set[Position](w, e, Position{7, 9})
	if got != (Position{7, 9}) {
...
func TestOnSetReceivesEntityID(t *testing.T) {
	w := flecs.New()
	var gotID flecs.ID
	flecs.OnSet[Position](w, func(e flecs.ID, _ *Position) { gotID = e })
	e := w.NewEntity()
	flecs.Set[Position](w, e, Position{1, 2})
	if gotID != e {
...
func TestOnAddFiresOnAddID(t *testing.T) {
	w := flecs.New()
	addCount := 0
	flecs.OnAdd[Tag](w, func(_ flecs.ID, _ *Tag) { addCount++ })
...
func TestOnAddNotFiredForCarriedComponents(t *testing.T) {
	...
	flecs.OnAdd[Position](w, func(_ flecs.ID, _ *Position) { posAddCount++ })
	flecs.OnAdd[Velocity](w, func(_ flecs.ID, _ *Velocity) { velAddCount++ })
	e := w.NewEntity()
	flecs.Set[Position](w, e, Position{1, 2}) // migrates: adds Position → OnAdd[Pos] fires
	flecs.Set[Velocity](w, e, Velocity{3, 4}) // migrates: adds Velocity → OnAdd[Vel] fires only
	if posAddCount != 1 {
...
func TestOnRemoveFiresBeforeRemoval(t *testing.T) {
	...
	flecs.OnRemove[Position](w, func(_ flecs.ID, p *Position) { captured = *p })
	e := w.NewEntity()
	flecs.Set[Position](w, e, Position{1, 2})
	flecs.Remove[Position](w, e)
...
func TestOnRemoveEntityStillAliveInCallback(t *testing.T) {
	w := flecs.New()
	alive := false
	flecs.OnRemove[Position](w, func(e flecs.ID, _ *Position) {
		alive = w.IsAlive(e)
	})
	e := w.NewEntity()
	flecs.Set[Position](w, e, Position{1, 2})
...
func TestOnRemoveFiresPerComponentOnDelete(t *testing.T) {
	...
	flecs.OnRemove[Position](w, func(_ flecs.ID, _ *Position) { posRemoved = true })
	flecs.OnRemove[Velocity](w, func(_ flecs.ID, _ *Velocity) { velRemoved = true })
	e := w.NewEntity()
	flecs.Set[Position](w, e, Position{1, 2})
...
func TestHookPanicPropagates(t *testing.T) {
	w := flecs.New()
	flecs.OnSet[Position](w, func(_ flecs.ID, _ *Position) {
		panic("hook panic")
	})
	e := w.NewEntity()
	defer func() {
...
func TestOnSetReadOnlyReentrancy(t *testing.T) {
	...
	var gotVel Velocity
	e := w.NewEntity()
	flecs.Set[Velocity](w, e, Velocity{5, 6})
	flecs.OnSet[Position](w, func(eid flecs.ID, _ *Position) {
		gotVel, _ = flecs.Get[Velocity](w, eid)
	})
	flecs.Set[Position](w, e, Position{1, 2})
...
func TestSetPairFiresPairTypeInfoHooks(t *testing.T) {
	...
	e := w.NewEntity()
	flecs.SetPair[Position](w, e, rel, tgt, Position{1, 2})
	if addCount != 1 {
	...
	flecs.SetPair[Position](w, e, rel, tgt, Position{3, 4})
	if addCount != 1 {
